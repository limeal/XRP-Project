...
async function mintAndToken(account: string, uri: string) {
...
async function createOfferNft(account: string, nftokenID: string) {
...
async function acceptOfferNft(account: string, offerId: string) {
    try {
        const result = await xumm.payload?.createAndSubscribe({
...
async function main() {
    ...
    const result = await mintAndToken(lewis, `ipfs://${ipfsHash}`);
    ...
    const txId = await createOfferNft(lewis, nftokenID);
    ...
    await acceptOfferNft(limeal, offerId);
}
